...
<body>
    ...
    <div class="period">
        Periode: {{ \Carbon\Carbon::createFromDate($year, $month, 1)->format('F Y') }}
    </div>

    @if($kehadirans->isEmpty())
        <p style="text-align: center;">Tidak ada data kehadiran untuk periode ini.</p>
    @else
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $kehadiran->waktu_hadir ? \Carbon\Carbon::parse($kehadiran->waktu_hadir)->format('d-m-Y') : '-' }}</td>
